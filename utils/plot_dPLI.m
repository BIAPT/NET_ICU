...
for ii = 1:4
    figure_title = strcat('dPLI-',ID,'-',frequency,'-',task,'-',hemisphere);
    figure_title_save = strcat('dPLI-',ID,'-',frequency,'-',task,'-',hemisphere,'-',num2str(ii));
    ...
    saveas(figure(ii), fullfile(outdir, figure_title_save), 'jpg')
    %pause(2);
    close(figure(ii))
